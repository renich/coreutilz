...
pub fn run(args: [][]const u8, allocator: std.mem.Allocator) !u8 {
    ...
    for (args[1..]) |arg| {
        if (parsing_options) {
            ...
            if (std.mem.startsWith(u8, arg, "--") and arg.len > 2) {
                try stderr.print("whoami: unrecognized option '{s}'\nTry 'whoami --help' for more information.\n", .{arg});
                return 1;
            }
            if (std.mem.startsWith(u8, arg, "-") and arg.len > 1) {
                try stderr.print("whoami: invalid option -- '{c}'\nTry 'whoami --help' for more information.\n", .{arg[1]});
                return 1;
            }
        }
    ...
    if (operands.items.len > 0) {
        try stderr.print("whoami: extra operand '{s}'\nTry 'whoami --help' for more information.\n", .{operands.items[0]});
        return 1;
    }
    ...
    if (pw == null) {
        try stderr.print("whoami: cannot find name for user ID {d}\n", .{uid});
        return 1;
    }

    stdout.print("{s}\n", .{std.mem.span(pw.*.pw_name)}) catch return 1;
    stdout.flush() catch return 1;
    return 0;
}
