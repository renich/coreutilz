...
test "tail basic (default 10 lines)" {
...
test "tail -c (bytes)" {
    ...
    try testing.expectEqual(@as(u8, 0), result.exit_code);
    try testing.expectEqualStrings("67890", result.stdout);
}

test "tail multiple files" {
    const allocator = testing.allocator;
...
test "tail --version" {
    ...
    try testing.expectEqual(@as(u8, 0), result.exit_code);
    try testing.expect(std.mem.containsAtLeast(u8, result.stdout, 1, "tail"));
}

test "tail nonexistent file" {
    const allocator = testing.allocator;
